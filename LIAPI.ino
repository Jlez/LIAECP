...
int Error;                                // value to show error
float PGain;                              // value to show P
float IGain = 0.1;                        // value to show I
int integralAction;                       // value for integral action
unsigned long previousMillis = 0;         //value for time
const long interval = 100;                // interval between measurements
unsigned long currentMillis;              // value for current time
...
if (STATERight == 0) {                                    // Check if the PB is pressed and held
    CountP = CountP - 1;                              // Remove 5 from the count
...
if (currentMillis - previousMillis >= interval) {       // formula to determine what the output should be
...
// Display all that is below to the serial monitor
Serial.print("PV= ");
Serial.println(PV);
Serial.print("\t");
Serial.print("SP= ");
